...
debug_logger(gchar const *log_domain,
             ...
             gpointer user_data)
{
        printf("LEADER(main): %s\n", message);
        syslog(LOG_INFO, "LEADER(main): %s", message);
}
...
#ifdef USE_OPENRC
                uid_t uid = getuid();
                g_autofree char *session_bus = g_strdup_printf ("unix:path=/run/user/%d/bus", uid);
                g_warning ("No session bus running! Using the hardcoded one for OpenRC...");
                g_setenv ("DBUS_SESSION_BUS_ADDRESS", session_bus, TRUE);
#else
